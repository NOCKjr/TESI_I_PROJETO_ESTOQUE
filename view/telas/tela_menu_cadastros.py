...
class TelaMenuCadastros(TelaBase):
    def __init__(self, master, gerenciador_de_janelas: GerenciadorDeJanelasBase, largura=constants.LARGURA_JANELA, altura=constants.ALTURA_JANELA):
        ...
        self.btn_continuar = ttk.Button(self.container_visual, text='Cadastrar Usuário', command=lambda: self.alterar_para_a_tela(constants.TELA_CADASTRAR_USUARIO))
        self.btn_continuar.grid(row=0, column=0, columnspan=1, sticky='nswe', padx=2, pady=2)
        
        ## Botão Cadastrar Escola
        self.btn_continuar = ttk.Button(self.container_visual, text='Cadastrar Escola', command=lambda: self.alterar_para_a_tela(constants.TELA_CADASTRAR_ESCOLA))
        self.btn_continuar.grid(row=1, column=0, columnspan=1, sticky='nswe', padx=2, pady=2)
        
        ## Botão Cadastrar Fornecedor
        self.btn_continuar = ttk.Button(self.container_visual, text='Cadastrar Fornecedor', command=lambda: self.alterar_para_a_tela(constants.TELA_CADASTRAR_FORNECEDOR))
        self.btn_continuar.grid(row=2, column=0, columnspan=1, sticky='nswe', padx=2, pady=2)
        
        ## Botão Cadastrar Insumo
        self.btn_continuar = ttk.Button(self.container_visual, text='Cadastrar Insumo', command=lambda: self.alterar_para_a_tela(constants.TELA_CADASTRAR_INSUMO))
        self.btn_continuar.grid(row=3, column=0, columnspan=1, sticky='nswe', padx=2, pady=2)
        
        ## Voltar ao Menu
        self.btn_continuar = ttk.Button(self.container_visual, text='Voltar ao Menu', command=lambda: self.alterar_para_a_tela(constants.TELA_MENU_PRINCIPAL))
